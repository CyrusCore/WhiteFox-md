...
`.trim()  
        ...
        case 'pet':
            ...
            let unicorn = global.db.data.users[m.sender].kuda
            let _pet = `${pickRandom(['kucing', 'rubah', 'kuda', 'unicorn'])}`.trim()
            if (global.db.data.users[m.sender].petcrate > 0) { 
                global.db.data.users[m.sender].petcrate -= 1
                if (_pet == 'kucing' && kucing > 0) {
                    ...
                    global.db.data.users[m.sender].makananpet += mknp * 1
                    conn.reply(m.chat, `*Selamat Anda mendapatkan pet ${_pet}${mknp > 0 ? ` Dan ${mknp} Makanan Pet*` : '*'}`, m)
                } else if (_pet == 'unicorn' && unicorn == 0) {
                    global.db.data.users[m.sender].unicorn += 1
                    global.db.data.users[m.sender].makananpet += mknp * 1
                    conn.reply(m.chat, `*Selamat Anda mendapatkan pet ${_pet}${mknp > 0 ? ` Dan ${mknp} Makanan Pet*` : '*'}`, m)
                } else {
                    global.db.data.users[m.sender].makananpet += mknp * 1
