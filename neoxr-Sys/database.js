module.exports = (m) => {
    ...
    let user = global.dbStorm.users[m.sender]
    if (user) {
    ...
    if (m.isGroup) {
       let group = global.dbStorm.groups[m.chat]
       if (group) {
    ...
    let setting = global.dbStorm.setting
    if (setting) {
