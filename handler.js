    ...
    try {
      ...
      try {
        ...
        if (user) {
          ...
          if (!isNumber(user.petcrate)) user.petcrate = 0;
          ...
          if (!isNumber(user.ghost)) user.ghost = 0;
          if (!isNumber(user.ghostexp)) user.ghostexp = 0;
          ...
          if (!isNumber(user.ghostlastfeed)) user.ghostlastfeed = 0;
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || "")[0])) {
          ...
          if (plugin.mvpp && !isMvpp) {
            //MVP+ COEG SUGIH WONG E
            fail("mvpp", m, this);
            continue;
          }
          ...
          if (
            ...
            this.reply(
              m.chat,
              `Your limit is up, please buy via *${usedPrefix}shopxp*`,
              m
            );
          ...
          if (plugin.level > _user.level) {
            this.reply(
              m.chat,
              `level required ${plugin.level} to use this command. Your level ${_user.level}`,
              m
            );
...
global.dfail = (type, m, conn) => {
  let msg = {
    restrict: "This command has been *Disabled* !",
    ...
    vip: "Please Buy Vip Role To Access This Command",
    mvp: "Please Upgrade Your Role Or Buy The Mvp To Access This Command",
    mvpp: "Please Upgrade Your Role Or Buy The Mvp+ To Access This Command",
  }[type];
